...
export type WhenMatch<R, P> = [
    0
] extends [1 & R] ? PForMatch<P> : P extends SafeRefinement<infer SP, never> ? SP : P extends Refinement<infer _R, infer RP> ? [Extract<R, RP>] extends [infer X] ? [X] extends [never] ? RP : X : never : P extends PredicateA<infer PP> ? PP : ExtractMatch<R, PForMatch<P>>;
/**
 * @since 1.0.0
 */
...
type SafeRefinementP<A> = A extends never ? never : A extends SafeRefinement<infer S, infer _> ? S : A extends Function ? A : A extends Record<string, any> ? DrainOuterGeneric<{
    [K in keyof A]: SafeRefinementP<A[K]>;
}> : A;
type SafeRefinementR<A> = A extends never ? never : A extends SafeRefinement<infer _, infer R> ? R : A extends Function ? A : A extends Record<string, any> ? DrainOuterGeneric<{
    [K in keyof A]: SafeRefinementR<A[K]>;
}> : A;
type ResolvePred<A> = A extends never ? never : A extends Refinement<any, infer P> ? P : A extends Predicate<infer P> ? P : A extends SafeRefinement<any> ? A : A extends Record<string, any> ? DrainOuterGeneric<{
    [K in keyof A]: ResolvePred<A[K]>;
}> : A;
type ToSafeRefinement<A> = A extends never ? never : A extends Refinement<any, infer P> ? SafeRefinement<P, P> : A extends Predicate<infer P> ? SafeRefinement<P, never> : A extends SafeRefinement<any> ? A : A extends Record<string, any> ? DrainOuterGeneric<{
 ...
 * @since 1.0.0
 */
export type PatternBase<A> = A extends ReadonlyArray<infer _T> ? ReadonlyArray<any> | PatternPrimitive<A> : A extends Record<string, any> ? Partial<DrainOuterGeneric<{
...
type Last<U extends any> = IntersectOf<U extends unknown ? (x: U) => void : never> extends (x: infer P) => void ? P : never;
type _ListOf<U, LN extends Array<any> = [], LastU = Last<U>> = {
    0: _ListOf<Exclude<U, LastU>, [LastU, ...LN]>;
    1: LN;
}[[U] extends [never] ? 1 : 0];
type ListOf<U extends any> = _ListOf<U> extends infer X ? X extends Array<any> ? X : [] : never;
...
type ExtractAndNarrow<I, P> = unknown extends P ? I : IsUnion<I> extends true ? ListOf<I> extends infer L ? L extends Array<any> ? Exclude<DrainOuterGeneric<{
    [K in keyof L]: ExtractAndNarrow<L[K], P>;
}>[number], Fail> : never : never : I extends ReadonlyArray<any> ? P extends ReadonlyArray<any> ? DrainOuterGeneric<{
    readonly [K in keyof I]: K extends keyof P ? ExtractAndNarrow<I[K], P[K]> : I[K];
}> extends infer R ? Fail extends R[keyof R] ? never : R : never : never : IsPlainObject<I> extends true ? string extends keyof I ? I extends P ? I : never : symbol extends keyof I ? I extends P ? I : never : Simplify<DrainOuterGeneric<{
