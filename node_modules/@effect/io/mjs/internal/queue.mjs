...
/** @internal */
export const bounded = requestedCapacity => core.flatMap(queue => make(backingQueueFromMutableQueue(queue), backPressureStrategy()))(core.sync(() => MutableQueue.bounded(requestedCapacity)));
/** @internal */
export const dropping = requestedCapacity => core.flatMap(queue => make(backingQueueFromMutableQueue(queue), droppingStrategy()))(core.sync(() => MutableQueue.bounded(requestedCapacity)));
/** @internal */
export const sliding = requestedCapacity => core.flatMap(queue => make(backingQueueFromMutableQueue(queue), slidingStrategy()))(core.sync(() => MutableQueue.bounded(requestedCapacity)));
/** @internal */
export const unbounded = () => core.flatMap(queue => make(backingQueueFromMutableQueue(queue), droppingStrategy()))(core.sync(() => MutableQueue.unbounded()));
/** @internal */
const unsafeMake = (queue, takers, shutdownHook, shutdownFlag, strategy) => {
  return new QueueImpl(queue, takers, shutdownHook, shutdownFlag, strategy);
};
/** @internal */
export const make = (queue, strategy) => core.map(deferred => unsafeMake(queue, MutableQueue.unbounded(), deferred, MutableRef.make(false), strategy))(core.deferredMake());
/** @internal */
